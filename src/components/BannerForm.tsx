...
import { createBanner, updateBanner } from '@/app/dashboard/banners/actions'
...
export default function BannerForm({ initialData }: { initialData?: any }) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    // Override image_url with our uploaded URL
    ...
    <div className="max-w-3xl space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <form onSubmit={handleSubmit} className="bg-zinc-900/50 border border-zinc-800/50 rounded-2xl p-6 md:p-8 space-y-6 backdrop-blur-xl">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="space-y-2">
            ...
            <input 
              ...
              placeholder="e.g. Semester Exam Preparation"
          ...
          <div className="space-y-2">
            <label className="text-sm font-medium text-zinc-300">Screen Name <span className="text-red-400">*</span></label>
            <select 
              name="screen_name" 
              defaultValue={initialData?.screen_name || 'home'} 
              required 
          ...
          <div className="space-y-2">
            ...
            <input 
              ...
              placeholder="e.g. Get ready for your upcoming exams"
              className="w-full rounded-lg px-4 py-3 bg-zinc-950 border border-zinc-800 text-white placeholder-zinc-600 focus:outline-none focus:ring-2 focus:ring-indigo-500/50"
            />
          </div>

          <div className="space-y-2">
            <label className="text-sm font-medium text-zinc-300">Priority (Higher is more important) <span className="text-red-400">*</span></label>
            <input 
              type="number"
              name="priority" 
        ...
        <div className="space-y-2 md:col-span-2">
          <label className="text-sm font-medium text-zinc-300">Banner Image <span className="text-red-400">*</span></label>
          ...
          <input 
            ...
            className="hidden"
          />
          {/* Hidden input to carry the URL into formData */}
            ...
            <div className="relative group rounded-xl overflow-hidden border border-zinc-700/50 bg-zinc-950">
              ...
              <div className="p-3 bg-zinc-900/90 border-t border-zinc-800/50">
                <p className="text-xs text-zinc-500 truncate">
                  <span className="text-zinc-400 font-medium">URL:</span> {imageUrl}
                </p>
              </div>
            </div>
          ) : (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="space-y-2">
            ...
            <input 
              ...
              placeholder="e.g. View Notes"
              className="w-full rounded-lg px-4 py-3 bg-zinc-950 border border-zinc-800 text-white placeholder-zinc-600 focus:outline-none focus:ring-2 focus:ring-indigo-500/50"
            />
          </div>

          <div className="space-y-2">
            <label className="text-sm font-medium text-zinc-300">Button URL</label>
            <input 
              name="button_url" 
              defaultValue={initialData?.button_url} 
              placeholder="e.g. /notes/exam-prep"
              className="w-full rounded-lg px-4 py-3 bg-zinc-950 border border-zinc-800 text-white placeholder-zinc-600 focus:outline-none focus:ring-2 focus:ring-indigo-500/50"
            />
          </div>
        </div>
