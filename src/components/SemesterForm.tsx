...
export default function SemesterForm({ 
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (initialData?.id) {
      ...
      result = await updateSemester(formData)
    } else {
    ...
    <div className="max-w-2xl space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex items-center gap-4">
        ...
        <h1 className="text-3xl font-bold tracking-tight text-white">
          {initialData ? 'Edit Semester' : 'New Semester'}
        </h1>
      </div>
      ...
      <form onSubmit={handleSubmit} className="bg-zinc-900/50 border border-zinc-800/50 rounded-2xl p-6 md:p-8 space-y-6 backdrop-blur-xl">
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            <label className="text-sm font-medium text-zinc-300">Semester Number <span className="text-red-400">*</span></label>
            <input 
              type="number"
              name="number" 
              defaultValue={initialData?.number || 1} 
              min={1}
              max={10}
              required 
              placeholder="e.g. 1"
